...
- **High concern (seen 5×).** Credibility factor is not assessed but model risk level exceeds 2 — unassessed factors at elevated risk weaken the credibility argument. Relates to: Evaluation methodology, Out-of-scope use, Bias and fairness analysis, Robustness and safety testing, Affected populations.
...
- **Critical concern (seen 8×).** Score informs a high-risk decision while carrying neither an uncertainty estimate nor a null baseline - there is no basis for judging whether the reported difference is real. Relates to: Score and uncertainty, Null calibration.
...
- **Critical concern (seen 9×).** Reported evaluation states no context-of-use relevance, and an assessment context was supplied - the score is being read against a decision the published record never claimed it informs. Relates to: Context-of-use relevance. _(common: most published models state no context of use)_
