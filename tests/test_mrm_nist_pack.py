...
sys.path.insert(0, str(_ROOT / "packs" / "model-credibility" / "examples"))
...
def test_pack_loads_and_has_17_factors():
    paths.validate_active_packs(active=["model-credibility"])  # manifest schema + load gate
    assert len(expected_factors("model-credibility")) == 17


def test_factorfocus_names_are_real_model_credibility_factors():
    # A declared focus factor that isn't a real model-credibility factor is a silent authoring
    # typo the loader can only drop (parallels tests/test_weakener_focus for the
    # core/vv40/nasa packs, which don't cover model-credibility).
    #
    ...
    focus = paths.detection_config(paths.pack_manifest("model-credibility")).get("factorFocus") or {}
    assert focus, "model-credibility should declare a factorFocus map"
    universe = set(attributable_factors("model-credibility"))
    for pid, names in focus.items():
        for fac in names:
            assert fac in universe, f"model-credibility:{pid} declares unknown factor {fac!r}"
...
def test_group_b_factors_are_attributable_but_not_counted_for_completeness():
    ...
    attributable = set(attributable_factors("model-credibility"))
...
def test_each_card_partitions_all_17_factors():
    universe = sorted(expected_factors("model-credibility"))
    for card in CARDS:
...
def _bundle(card, tmp_path: Path) -> Path:
    doc = map_to_jsonld(build_import_dict(card), packs=["model-credibility"],
...
def test_r2_firing_gate(tmp_path):
    ...
    for card in CARDS:
        ...
        pats = frozenset(f["patternId"] for f in pipeline._run_weakeners(p, "model-credibility"))
...
def test_card_bundle_has_no_factortype_shacl_violation(tmp_path):
    # The central P0 gotcha: model-credibility factors must carry factorStandard so the vv40
    ...
    _conforms, violations = pipeline._run_check(p, "model-credibility")
