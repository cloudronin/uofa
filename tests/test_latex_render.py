...
@needs_tex
def test_hyphenation_is_present_but_in_the_real_range(rendered):
    """Two-sided. 31.9% is as wrong as 0% -- the real papers span 0.7-8.3%."""
    assert rendered["hyphen_lines"] >= L.TARGETS["hyphen_lines"]
    assert rendered["hyphen_lines"] <= 0.12
