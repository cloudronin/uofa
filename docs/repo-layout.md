...
| `tests/` | Pytest test suite + adversarial fixtures (~181 prompt snapshots) | Active |
...
| `out/` | Generated artifacts (gitignored). Includes `out/adversarial/` corpora and `out/phase2_5/` refinement outputs | Ephemeral |
| `docs/` | Operational documentation | Mixed |
| `corpus/` | Test corpus fixtures (edge-cases, import-tests) | Stable |
| `keys/` | ed25519 signing keys (research keys; production keys should be elsewhere) | Stable |
| `scripts/` | Miscellaneous shell scripts | Stable |
| `weakener-engine/` | Apache Jena rule engine subproject (Java; produces the JAR shipped in the wheel) | Stable |
...
The output workspace, all gitignored.

| Path | Purpose |
|---|---|
| `out/adversarial/phase2/<date>-<version>/` | Generated corpus dirs (M5 baseline + per-version regen + holdouts) |
| `out/adversarial/archive/` | Archived dev artifacts (SMOKE* dirs from pre-Phase-3) |
| `out/phase2_5/2026-04-27/` | Phase 2.5 refinement workspace (see `out/phase2_5/README.md`) |
...
| "What changed in v0.5.X?" | See per-version summary in `out/phase2_5/2026-04-27/` (e.g. `v0510_summary.md`); also `CHANGELOG.md` |
...
| "Where do generated NCs live?" | `out/adversarial/phase2/<date>-vX/negative_controls/` (gitignored) |
| "What did Phase 2.5 fix?" | M5 NC clean rate 0% → 97.2%. See `out/phase2_5/README.md` for the version chain |
