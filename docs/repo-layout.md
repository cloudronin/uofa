...
| `src/uofa_cli/solver/` | Simulation-artifact ingest — sniff by content, walk a Workbench `.wbpz` without unpacking it, digest every member, redact operator identity, and read the project file and materials library. Reads solver evidence with no vendor software; see `uofa evidence` | Active |
