...
| [INV-21](INV-21-claim-node-conventions.md) | OPEN | **The claim node is fully wired and completely empty, and the rules guard on a class that is declared nowhere.** `bindsClaim` has domain and range, is present in all 71 adjudication packages and all 6 canonical examples, and is already traversed by two core rules — and `uofa:AssuranceClaim` carries **0 declared properties**, so **seven incompatible conventions** grew (bare IRI in 71/71 corpus and the skeleton template; inline node in morrison/cou2; a pack-typed array in iso42001; and `hasSupportingEvidence`, which every OOS rule reads and **0 of 71** corpus packages carry — so the OOS subsystem is blocked by the same emptiness). The rules guard on `uofa:Claim` (3 sites), which is declared nowhere; `uofa:AssuranceClaim` appears in **0** rules. Consequence: the guard added in `205cc90e` (one day after the corpus date) silenced W-EP-01 on 63 packages, cascading to COMPOUND-01/03 — **63 of 65 comparable packages now differ from their recorded `rules_fired`** | open — two Stage 4 rows genuinely affected (#3, #65); rule-site fix is a bug independent of the [argument layer proposal](../UofA_Argument_Layer_Spec_v0_1.md), prototyped in `dev/prototypes/argument-layer/` |
| [INV-22](INV-22-oos-calibration-has-no-positive-cases.md) | OPEN | **The OOS calibration set contains no case that should pass.** All **16** OOS calibration packages are `out_of_scope` cases that should report a gap; **0** are sufficient bundles that should clear. So a rule that can only ever report a gap scores perfectly — and **10 of the 16** shipped rules were exactly that, needing 2–3 *distinct* `hasSupportingEvidence` bindings that the pre-fix engine collapsed onto one node. Every instrument agreed: 11/11 Java, 37/37 Python, and the load-bearing byte-identical baseline regression, whose 5 pinned baselines are all `out_of_scope` stubs | engine defect fixed in #81; the calibration gap is open — needs one sufficient package per rule, and manifest entries for the 11 OOS targets that have none |
