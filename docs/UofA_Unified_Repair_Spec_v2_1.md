...
**RESULT — GATE-H3 evaluated once at v0.5.15.1, 2026-08-16. It splits, and the split is reported as measured with no adjustment.**

| Clause | Gate | Measured | Arm | |
|---|---|---|---|---|
| MECHANICAL | ≥95% | **100.0%** (35/35 over the 13-pattern denominator) | M | **PASS** |
| Overall | ≥80% | **75.9%** (287/378 evaluable rows) | G | **FAIL** |

**The two clauses measure different things, and the gap is attributable rather than mysterious.** The overall figure is dragged by **W-AR-04, W-EP-03 and W-CON-03 at 0/20 each**, because the legacy LLM generator never builds the structures those rules read — and **the same three score 3/3 under Arm M once the structure exists.** That is the generator-versus-encoding disagreement this phase was built to expose, showing up in the gate arithmetic itself.

**A gate that fails on exactly the axis the mutation arm was built to expose is stronger evidence than a clean pass would have been**, because it shows the gate had teeth. Reported as measured; the catalog is not patched and nothing is re-run to improve it (fix-and-remeasure is a disclosed v0.6 event).

*Prior context, retained.* The best measurement before this was the v0.5.13 holdout: MECHANICAL 72.6% against ≥95%, overall 76.2%, JUDGMENT 91.6%. The gate was held rather than revised because every MECHANICAL zero was diagnosed as a generation artifact rather than a rule defect (INV-8/INV-11), and the mutation arm is what made ≥95% reachable by construction. That diagnosis is now confirmed: MECHANICAL reaches 100% the moment the defect is expressible. The gate's v2.0 justifying parenthetical — "(the holdout supports it)" — stays withdrawn.

**The version-consistent pair, replacing the mismatched one.** Both figures now at v0.5.15.1:

| | value | denominator |
|---|---|---|
| CE recall | **75.9%** | 287/378 **evaluable rows** |
| NC clean rate | **97.1%** | 171 validated |

This retires `73.4% @ v0.5.7 beside 97.1% @ v0.5.15.1`, which A2 §3's own null-control standard forbids.

> **Name the denominator, every time.** `68.6%` and `76.2%` already in the record are **the same measurement** — 288/420 against 288/378. The 42-package difference is the entire CE corpus for W-ON-01 and W-SI-01, `shaclFailed: 20` on both specs. The manuscript's M5 figure is quoted as "73.4% (evaluable rows)", so the exclusion convention is followed here and stated. **Any sentence carrying one of these numbers without its denominator inherits the ambiguity**, and two of them differ by 7.6 points while describing an identical run.

> **One package separates the versions, and it is attributable.** W-CON-01 goes 20/20 → 19/20; every other pattern's count is identical. The cause is the single rule-body change at v0.5.14, `notEqual(?status,'not-assessed')`. Worth recording because `rules:376-387` argues that guard is recall-neutral — *"no CE target uses 'not-assessed' as the trigger."* Measured: one in twenty. A small correction, but it is the difference between an argued property and a measured one.
...
| less no-gate-eligible-mutant | **13** | W-SI-01, W-ON-01, W-SI-02 | Their defect necessarily breaks the profile, so **zero conformant-but-flawed mutants exist** (measured, `studies/phase2_5a/conformance.json`). Under addendum E only conformant mutants score. **The rules do detect them** on non-conformant mutants — both layers catch independently |

Addendum E scores the gate on conformant-but-flawed mutants only, and that rule is about **threat realism**: a package failing schema validation gets bounced at intake, so detecting one says little about the dangerous case, which conforms and is still wrong. Those three admit no conformant mutant at all, so they have nothing gate-eligible to score — which says nothing about rule quality, since the rule engine demonstrably detects them. Scoring them as zeros would make the gate measure what the profile happens to mandate rather than what the catalog detects, and a gate that cannot mathematically pass regardless of rule quality is a foregone conclusion wearing one.

> **Corrected 2026-08-16, before A4.** An earlier version of this passage said the completeness profile "intercepts before C3 runs." **It does not** — `check.run_structured` runs C2 → C1 → C2.5 → C3 with no early return and no conformance guard, and 12 of 23 Class A mutants are measured non-conformant *and* rule-layer-caught. The arithmetic was unaffected; the stated mechanism was falsifiable with one CLI invocation, which is exactly where a committee member would have found it.
...
2. **Mutator (`uofa inject`):** deterministic mutations over the **17-pattern MECHANICAL set** (the 21-pattern figure is the *base* partition, not the MECHANICAL count — see §0.1). Two operator classes per the falsification, split as **measured** rather than projected (`fed5a37e`, revised at `87463323` when W-PROV-01 moved A→B on measurement): **Class A**, single edit to a field the substrate already has, **8 patterns**; **Class B (MUT-ANT-\*)**, antecedent instantiation plus violation, two edits carrying one fault, **9 patterns**, each mutant flagged `enrichment: true` so the rollup can be split. Class A is measured complete at v0.5.15.1: 23 mutants, of which **11 conformant-but-flawed across 5 patterns** are gate-eligible and all fire; the other 3 patterns produce zero conformant mutants, which is the measured basis for addendum F's exclusions. The enrichment family doubled against plan — a standing scope escalation the author resolves, not the implementation. Two rule findings ride on it: W-EP-01 cannot fire on schema-conformant evidence (its guard names a class the schema never defines), and W-ON-02 already fires on all three case studies, so its recall needs an enrich-to-clean operator. The MUT-TYP datatype family is **deleted** — it was built on the falsified theory. MUT-INT-02 is **dropped as a finding**, not worked around: the issuer path refuses to sign synthetic packages, which is the positive architectural claim that the production signing path cannot produce a fraudulent-but-valid package. Manifest derived from the canonical diff, never operator intent; liveness check excludes canonicalization-erased mutants from the denominator; mutants load through the production path (emittability rule).
...
5. **CLI + walkthrough (B2):** `uofa inject` + `uofa inject verify`, with `uofa rules` as the detection step (`detect` deleted — it duplicated `rules` and its `--manifest` path would have let the detector see the answer key). Fresh-clone README walkthrough with the three letter-named demos (remove uncertainty, change version numbers, remove signatures).
6. ~~**P25-A at v0.5.15.1, two arms**~~ **DONE 2026-08-16.** Arm M (`008626f2`) and Arm G (`e40d7819`); report at `studies/phase2_5a/REPORT.md`. GATE-H3 evaluated once and **splits** — MECHANICAL 100% over 13, overall 75.9% against ≥80% — reported as measured, no adjustment (§0.1). The version-mismatched 73.4%/97.1% pair is retired, replaced by 75.9%/97.1% both at v0.5.15.1.
...
| **Issue #67 guard** | **Conditional, not scheduled.** The loud-failure half landed 2026-08-17 — `analyze` now names the specs whose `spec_path` will not resolve and states that the per-COU columns are EMPTY rather than zero. The **path repair stays parked**. It fires only if a coverage-delta column (`recall_morrison_cou1/cou2`, `recall_min_per_cou`, `recall_cou_disparity`, `cou_dependent_flag`) reaches a chapter table — same shape as the contrast-variant rule at `studies/phase2_5a/REPORT.md:296`: build it before the claim is printed. | Any writing position that quotes a per-COU figure |
...
> **A4 disclosure, found and corrected (`a99e8b4f`, 2026-08-17): the NC evidence chain crosses a corpus boundary without marking it.** The cumulative chain at `holdout_v0515_summary.md:106-116` reads *M5 v0.5.7 0/176 → v0.5.12 175/180 → v0.5.13 holdout 14/16 → v0.5.15.1 holdout 166/171*, and labels its second row's source **"M5 corpus."** That row was measured on a **hybrid** batch — M5 with placeholder stubs inserted by the three tools in `dev/tools/phase2_5/corpus_regen/` — not on M5. The unpatched M5 corpus at v0.5.15.1 scores **8/176 = 4.5%**, so rows 1 and 2 are not the same corpus and the chain reads as a pure catalog trajectory when it is not. **This single unmarked boundary is where the 0% → 97.1% conflation entered**; every downstream statement inherits it. Decomposition and per-rule evidence at [INV-16](investigations/INV-16-nc-trajectory-decomposition.md). Handled in the house style: **the frozen 2026-04-29 artifact is untouched** and the scope note is carried by the live copies (`PHASE2_5_STATUS_REPORT.md`, `PHASE2_STATUS_REPORT.md`, `studies/phase2_5a/REPORT.md`), so the archive still says what it said and the working documents say what is true. **The primary record was honest** — `v0512_summary.md` labels each fix "predicate tighten" or "corpus regen" at the point of decision and states the diagnosis outright — so this is a rollup defect, not a data defect, which is the strongest form the correction can take.

> **A4 disclosure, quoting ban: the v0.5.12 hybrid NC corpus is not schema-valid today.** `dev/build/adversarial/phase2/2026-04-29-v0512/` was built by inserting placeholder `hasSensitivityAnalysis` stubs into every Complete-profile NC. Those stubs are the **inline-object** form, which the v0.5.15.1 SHACL rejects — the same D2 boolean mismatch that Phase 2.5 later fixed — so **31 of 180 packages read non-conformant under the current schema**. The corpus remains valid evidence for the one thing it is cited for in [INV-16](investigations/INV-16-nc-trajectory-decomposition.md): its clean rate against the v0.5.7 catalog is 0.0% on **every** evaluable row, and the excluded 31 cannot change a floor of zero. **No other figure is quoted from it, and none should be.** Recorded so a reader who finds those packages learns why nothing cites them, rather than inferring an oversight.

> **A4 hazard, ~~recorded not fixed~~ FIXED (`548224d1`, 2026-08-16), entry retained because A4 discloses it: the re-derivation command returned nothing and exited non-zero.** `uofa adversarial analyze` yields **zero rows** on the committed Phase 2 corpora, because `batch_manifest.json` records `out_dir` as the pre-rename `out/adversarial/…` path. All 40 per-spec manifests are committed and intact; only the roll-up's pointers are stale. This matters for A4 **specifically**, because an audit appendix invites a reader to re-derive the numbers, and the official command handed them an empty table.
> **Resolved, and one clause of the original entry was my error.** The fix re-anchors on `--in` when `out_dir` misses ([classifier.py:372](../src/uofa_cli/adversarial/classifier.py)), resolving **510/510** specs across all three corpora with no frozen artifact touched; validated at scale on M5 in [INV-15](investigations/INV-15-m5-scale-and-phase3-gap-probes.md). **The "exits 0" half was never true** — `analyze` returns 1 on an empty result and always did; every observation of a zero exit came through `… | tail`, which reports the pipeline's last command. See the correction in [INV-14](investigations/INV-14-analyze-pointer-fix.md). The entry stays in A4 because the appendix discloses the defect and its repair, not only defects that survive; **the fix restores re-derivability of the published figures from the committed corpora**, which is the property A4 exists to offer.

> **Chapter material, not an appendix entry: the silent-null catalogue.** **Eight** occasions where a clean-looking result was wrong and **nothing announced it** — a conformance reading from a stale branch (0/23 then 23/23, both plausible), a flat probe on `@graph`-form packages reading 17 findings as detections, an intercept mechanism inferred from a shapes file and falsifiable by one CLI call, `analyze` returning no rows on a stale pointer, a validity predicate that asked the same question twice, an exit code read through `| tail` so the shell reported *tail's* success, five per-COU columns silently empty on every committed corpus, and **a fully green regression suite that was testing a different checkout than the branch being edited**. The canonical table is `docs/investigations/INV-6-findings.md`; the count here was stale at five and is corrected. The argument this supports: the praxis claims machine-checkable evidence beats prose-borne evidence, and the honest qualifier is that **machine-checkable is not self-validating**. A pipeline returning a plausible number from a stale branch, a renamed directory, or a mis-specified predicate has produced evidence that is machine-readable **and wrong**. The defence is not more automation — it is checking the instrument against a case whose answer is known independently, which is the mutation arm's own argument applied to the harness that measures it. That belongs in the manuscript, and it is earned rather than asserted.
