...
> **A4 hazard, recorded not fixed (`7b7a4b59`): the re-derivation command returns nothing and exits 0.** `uofa adversarial analyze` yields **zero rows** on the committed Phase 2 corpora, because `batch_manifest.json` records `out_dir` as the pre-rename `out/adversarial/…` path. All 40 per-spec manifests are committed and intact; only the roll-up's pointers are stale. This matters for A4 **specifically**, because an audit appendix invites a reader to re-derive the numbers, and the official command currently hands them a clean exit and an empty table — which reads as "the numbers aren't there" rather than "the pointer is stale." The working join is documented in INV-6: snake_case roll-up carries intent/target, camelCase per-spec carries specId/shaclFailed, join on `spec_id` → directory name, **ignore `out_dir`**. Either fix the pointers or give A4's reader the join; do not leave the appendix pointing at a command that silently returns nothing.
