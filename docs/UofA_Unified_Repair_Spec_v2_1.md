...
> **Chapter material, not an appendix entry: the silent-null catalogue.** Five occasions in one day where a clean-looking result was wrong and **nothing announced it** — a conformance reading from a stale branch (0/23 then 23/23, both plausible), a flat probe on `@graph`-form packages reading 17 findings as detections, an intercept mechanism inferred from a shapes file and falsifiable by one CLI call, `analyze` exiting 0 with no rows, and a validity predicate that asked the same question twice. The argument this supports: the praxis claims machine-checkable evidence beats prose-borne evidence, and the honest qualifier is that **machine-checkable is not self-validating**. A pipeline returning a plausible number from a stale branch, a renamed directory, or a mis-specified predicate has produced evidence that is machine-readable **and wrong**. The defence is not more automation — it is checking the instrument against a case whose answer is known independently, which is the mutation arm's own argument applied to the harness that measures it. That belongs in the manuscript, and it is earned rather than asserted.
