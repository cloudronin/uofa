...
- **Note for Phase 3 reviewers:** any references to `baseline_firings_count` in archived smoke-output CSVs (e.g., `dev/build/adversarial/archive/SMOKE/`) predate the fix and should be ignored.
...
> _Populated from `dev/build/adversarial/phase2/2026-04-26/coverage/{outcomes,summary,matrix}.csv` and the rendered `index.html`._
...
uofa adversarial prep-review \
  --outcomes dev/build/adversarial/phase2/2026-04-26/coverage/outcomes.csv \
  --output   dev/build/adversarial/phase2/2026-04-26/review_packets/ \
  --max-cases 50
```
