# UofA Author Decision Record 2026-08-19

Status: RULED
Owner: Vishnu Vettrivel
Consumers: Claude Code (Ch4 Numbers and Repairs follow-ups), A4 audit-trail appendix, parent spec UofA_Ch4_Numbers_and_Repairs_Spec_v1_0.md.
Succeeds the 2026-08-16 record, which is not reopened; this record is committed **alone**, per the same convention and for the same reason.
...
Every ruling here — R1a, R1b, R1c, R2, R3, R4, R5, and the §4.3.6 chapter
placement — is recorded with its date **before the manuscript prose it governs**.
None of the writing-queue positions these rulings bind has been drafted.

**A4 cites this file by its commit, not by its path**, for the reason the
predecessor states: an untracked file carries an mtime, not a commit date, and an
mtime is not evidence of ordering. This record is therefore committed **alone**,
which is also why the 2026-08-16 record was not reopened to append these rulings.
That record's audit note rests on its having been committed alone; appending to it
now would spend the property it exists to demonstrate.

**The ordering is checkable in both directions here**, which the predecessor could
not offer. Forward: the rulings precede the prose. Backward: the artifacts that
implement them already cite them by date and identifier — the W3 fix commit
(`bf116408`) names R1a in its message and its CHANGELOG entry, and the Stage 4
report (`b23622af`) carries R1b's basis statement verbatim. So the rulings can be
located from the record and the record can be located from the work, without
relying on either to vouch for itself.

**One methodological note the A4 appendix takes forward.** Two corrections in the
session that produced these rulings overrode plausible authored guesses, and they
cut in opposite directions: one trace found the record **worse** than assumed (a
test that never checked what it claimed), the other found it **better** (a figure
escalated as unlocatable that was correct and documented). A rule that defers to
the author catches neither. That is the appendix's thesis, and the reason the
governing spec's §2 verification rule binds the author's statements as well as the
session's.
...
### The two corrections, for the audit note above

The audit note's methodological paragraph rests on these two, recorded here with
their specifics.

**E1** — the spec's `0.964 vs 0.960` pair was escalated as unlocatable. It was
correct and documented; the escalation reflected not having found
`docs/extract_eval_v1.md`, which names itself the origin of the headline. The
trace found the record **better** than the session had assumed.

**`992955ac`** — the aero wiring breakage was attributed to the pack-directory
refactor. That commit predates the aero work entirely; the tests were mispointed
from the day they were written. The trace found the record **worse** than the
author had assumed: not a regression, but a check that never checked.
