...
| **3 — Triage** | ~~**NOT STARTED** (pilot-scale only)~~ **DONE.** Full-corpus triage ran: 4,556 cases bucketed, 21 DISAGREEMENT / 4,535 CONVERGENT (`dev/build/adversarial/phase3/triage/triage_summary.json`), 0.46% disagreement | Pilot triage exists (above). Full-corpus CONVERGENT/DIVERGENT/UNCERTAIN bucketing: **none**. `triage.py` built (100% cov per handoff). | — (`triage.py` is code, not output) | No corpus-level queue size; `arbitration_partition` empty in pilot. |
| **4 — Adjudication + agreement stats** | ~~**NOT STARTED**~~ **DONE 2026-08-19.** Author adjudication 71/71; full-corpus Cohen's κ (AB 0.6945, AC 0.7595, BC 0.8534), Fleiss 0.7685, confusion matrices, all at N=4,556. Weighted spot-check override rate 0.213 vs ≤0.10 — **FAIL as measured**. Report: `studies/phase3_stage4/REPORT.md`; re-derive with `studies/phase3_stage4/rederive_stage4.py` | None. Calibration κ exists (Stage 1) but **no full-corpus** Cohen's/Fleiss κ, confusion matrices, or author-vs-judge matrices. | — (`adjudication.py`, `arbitration.py` are code; no output data found after sweeping `dev/`, `runs/`, `harness/`, home) | No author adjudication records, no self-blinding / `post_hoc_alignment` data. |
