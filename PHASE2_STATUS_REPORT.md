...
1. **Catalog version skew between the corpus of record and the refined catalog — the central cross-phase risk.** The 2026-04-26 corpus (consumed by Phase 3) was generated/analyzed against the **v0.5.0 / M5-baseline (v0.5.7)** catalog, whose **NC precision was 0% (FPR 100%)**. Phase 2.5 then refined the catalog to **v0.5.15.1 (97.1% NC clean)** — a substantial change to which rules fire. ~~**No full-battery re-run on the refined catalog exists.**~~ **SUPERSEDED 2026-08-17 (`98959943`): the full battery has been re-analyzed at v0.5.15.1** — all four intents, 4,601 packages (confirm_existing 4,002 / gap_probe 330 / negative_control 179 / interaction 90), at zero LLM cost, because adversarial packages are catalog-version-independent and re-analysis is Jena time. The `$386 / 14h` in P2-B is for *regenerating* the corpus and never applied to this. **So the figure-scoping half of this risk is closed**: every M5 number now has a v0.5.15.1 counterpart and the chapter cites the version-consistent pair. What re-analysis cannot touch is Phase 3's §6.7 REAL-GAP adjudications, which were made against the pre-refinement corpus — see the P2-A ruling below.

2. **`precision = 0.0%` is the headline M5 number and reads catastrophically out of context.** It means every M5 negative control fired ≥1 rule on the *pre-refinement* catalog. Stated alone it undercuts the catalog; stated with the Phase 2.5 trajectory (0% → 97.1% NC clean) it becomes the motivating problem the work solves. Never present the M5 precision without the Phase 2.5 fix in the same breath.
   **Sharpened 2026-08-17 ([INV-16](docs/investigations/INV-16-nc-trajectory-decomposition.md)):** "clean negative control" overstates it, and the correction improves the number rather than excusing it. M5's NCs are negative only in that **no defect was deliberately planted** — 161 of 179 genuinely carry no applicability bounds on their Context of Use. W-ON-02 firing on 158 of 176 is therefore **correct detection of a real, uninjected weakness**, not a false positive, and the same rule fires on all three published case-study encodings. So `precision = 0.0%` is a mislabelled detection result: at v0.5.15.1 the unpatched M5 corpus still scores 4.5% "clean," because the packages have not changed and the absences are real. Present it as evidence that unbounded contexts of use are endemic in real evidence, which is the claim the catalog exists to support.
