...
- `uofa/v0.2` — **not a release tag.** It predates the `vN.N.N` scheme and is not published, not on PyPI, and not a reproducible baseline for anything. Do not resolve a version, pin a dependency, or source a comparison against it. Released versions are the `vN.N.N` tags only.
