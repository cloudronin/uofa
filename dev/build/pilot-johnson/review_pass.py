...
# The minting namespace. Identifier and Compares To are @type:@id terms in the v0.5
# context, so a relative value is dropped at JSON-LD expansion and the reference never
# reaches the graph. An absolute IRI is therefore required, and an absolute IRI cannot
# be written until the base_uri is chosen. Changing the namespace means editing this
# constant and re-running, which is the coupling made explicit rather than hidden.
BASE_URI = "https://github.com/cloudronin/uofa"
COU_SLUG = ("fod-tire-puncture-regression-model-aircraft-tire-penetration-risk-assessment/"
            "predict-aircraft-tire-fod-penetration-depth-to-support-probabilistic-risk-"
            "assessment-pra-of-a-ground-hold-decision")


def uri(kind: str, local: str) -> str:
    return f"{BASE_URI}/{COU_SLUG}/{kind}/{local}"


# ── Fourth-verb additions (protocol §3c) ─────────────────────────────────
# Confirm, correct and mark-source-absent all act on a cell that already exists.
# Outline v3's prompt 3c makes adding an evidence row a governed review act when a
# source anchor supports it, which closes the pilot's A-24 escalation. Each row
# below names the firing it clears and the anchor that licenses it.
#
# Only two are added. The other firings need evidence the workbook cannot express
# (SensitivityAnalysis has no route through the on-ramp) or name comparators that
# are not the kind of thing a URI identifies (SME judgment, a review panel, and
# the absence of RWS data). Adding a row for those would fabricate an identifier.
ADDITIONS = [
    # sheet, values by column index, anchor, note
    ("Validation Results",
     {1: "Formal DOE development and analysis process",
      2: "ProcessAttestation",
      3: uri("evidence", "process-attestation-doe"),
      4: "Standard formal experimental design and data analysis process rigorously "
         "followed and documented in standard form. Statistical metrics used to select "
...
# prose, so import dropped all five as non-well-formed subjects (A-21). Two comparators
# ARE entities this package already carries, so they get local identifiers and the
# reference is repaired. Three are not entities and stay as they are.
IDENTIFIERS = [
    ("Model & Data", 5, 3, uri("dataset", "dataset-doe-fod-impact-test-matrix"),
     "p.12 4.1.2 a; p.14 note",
     "Absolute identifier minted so the comparator reference below resolves. A relative "
     "one is silently dropped at expansion."),
    ("Validation Results", 4, 5, uri("dataset", "dataset-doe-fod-impact-test-matrix"),
     "p.21 [M&S 28](3); p.22 M&S Results",
     "Compares To repointed from prose to the dataset entity this package carries. "
     "Measured: clears the W-AR-05 firing on this row (D-03)."),
    ("Credibility Factors", 20, 8, uri("evidence", "process-attestation-doe"),
     "p.25 M&S Process / Product Management rationale",
     "Linked Evidence on Development process and product management, pointing at the row "
     "added under §3c. Measured: clears W-NASA-03 (D-10), which needs uofa:hasEvidence on "
     "the factor rather than the attestation merely existing."),
]
...
def main() -> None:
    ...
    # Fourth-verb additions and identifier repairs (§3c).
    for sheet, values, anchor, note in ADDITIONS:
        ws = wb[sheet]
        acol = _anchor_col(ws)
        row = ws.max_row + 1
        while row > 1 and all(ws.cell(row - 1, c).value in (None, "") for c in range(1, acol)):
            row -= 1
        for col, value in values.items():
            ws.cell(row, col).value = value
        ws.cell(row, acol).value = anchor
        tally["ADD"] = tally.get("ADD", 0) + 1
        ledger.append((sheet, f"row {row}", "ADD", None, values.get(1), anchor, note))

    for sheet, row, col, value, anchor, note in IDENTIFIERS:
        ws = wb[sheet]
        before = ws.cell(row, col).value
        ws.cell(row, col).value = value
        tally[CORRECT] += 1
        ledger.append((sheet, f"{chr(64+col)}{row}", CORRECT, before, value, anchor, note))
    ...
    lines = ["# Review ledger — every cell decision, generated by review_pass.py",
             ...
             f"(source-absent) · {tally.get('ADD', 0)} added (§3c) · "
    ...
    print(f"confirmed {tally[CONFIRM]}   corrected {tally[CORRECT]}   "
          f"blanked {tally[BLANK]}   added {tally.get('ADD', 0)}   "
