# `dev/tools/phase2_5/` — Phase 2.5 catalog refinement tooling
...
| Goal | Entry point |
|---|---|
| Run the refinement loop on a rule | `python -m dev.tools.phase2_5.refine_loop --rule W-EP-01` |
| Lock in metrics after a corpus regen / predicate edit | `python -m dev.tools.phase2_5.lock_in --rule <ID> --new-outcomes <csv>` |
| Patch the NC corpus (W-ON-02 / W-AR-02 / W-CON-04) | `python -m dev.tools.phase2_5.regen_nc_envelope` etc. |
| Audit a fresh-generated corpus for compliance | `python dev/tools/phase2_5/audit_phase_b.py <corpus-dir>` |
| Inspect cases the rule misfired on | `python -m dev.tools.phase2_5.inspect_misfires` |
| Plot PR trajectory across iterations | `python -m dev.tools.phase2_5.plot_pr --rule <ID>` |
...
Run with: `pytest dev/tools/phase2_5/tests/`
