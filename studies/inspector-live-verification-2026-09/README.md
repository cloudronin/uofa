...
| Package hash prefix | `3084b1c9` (first 8 hex of `packageHash`) |
| SHA-256 of the ZIP | *pending — see §8* |
...
| Field | Value |
|---|---|
| Executed at (UTC) | *pending — see §8* |
| Verifying CLI version | *pending — see §8* |
| Tool version that built the pack | *pending — `toolVersion` in the package's `MANIFEST.json`* |
...
## 8. Fields still to be filled

Three values live only on the verifying machine. Until they land, this record is
v1.0-provisional in those three rows.

```bash
# 1. digest of the exact file that was verified
shasum -a 256 uofa-pack-vv40-20260909T001330Z-3084b1c9.zip

# 2. version of the CLI that ran the verification
uofa --version

# 3. version of the tool that BUILT the pack, recorded inside it
python3 -c "import json;print(json.load(open('uofa-pack-vv40-20260909T001330Z-3084b1c9/MANIFEST.json'))['toolVersion'])"
```

Also needed: the UTC wall-clock time of the verification run (§5). The package
creation time in §3 is exact and server-generated; the verification time is not
recoverable from the artifact.

When the ZIP is placed in `packages/`, regenerate the digest manifest:

```bash
cd studies/inspector-live-verification-2026-09
shasum -a 256 packages/*.zip > SHA256SUMS
```
