...
|---|---:|---:|---|
| **keyless route** | **0/24 (0%)** | 1/1 — see below | none |
| `Llama-3.3-70B` (v2) | 2/24 (8%) | 1/9 (11%) | API |
| `DeepSeek-V4-Pro` (v2) | 12/24 (50%) | 1/9 (11%) | API |

**Zero misses on 24 cases, no model, no key, no tokens, fully deterministic.**
It is the only configuration all session to clear the false-fire bar outright
...
## The one false-clear is a label error the route found
...
|---|---|---|
| table-borne (P2, some P5) | **keyless field read** | clears the bar, measured |
...
1. **Most P2 evidence is table-borne** — 24 of 33 positives. So the property with
   the largest positive class is largely solved by a route that costs nothing.
...
- **Not a full row.** It covers P2 table-borne. Whether a P5 chance-value column
  route is worth building is a separate question, and P5's labels need review
  first.
