...
:::caution[What COU 1's eleven are made of]
The counts above reproduce, but **9 of COU 1's 11 are vacuous**, and a reader should know which before drawing a conclusion from the comparison.

COU 1 references its three validation results as **bare IRIs**; COU 2 **inlines** its three as full objects. `W-AL-01`, `W-AR-05` and `W-EP-02` each test `noValue(?result, …)`, so against a bare IRI — a node carrying no properties at all — every one of them succeeds for want of anything to read, and fires on every result. That is 3 × 3 = 9 of COU 1's 11. Inline the same three results, changing nothing else, and those nine disappear.

**COU 2's 18 are substantive**; it has none of these. So the COU1-vs-COU2 contrast on the page is partly a difference in serialization, not only in evidence adequacy. The `COMPOUND-01` point stands — compound firings are unaffected — but "COU 1 is cleaner than COU 2" is not a reading these numbers support.

Measured 2026-08-16 at catalog v0.5.15.1; both packages verify, and both are profile-conformant before assessment. The rule behaviour is reported, not patched: v0.5.15.1 is frozen, and a guard for these three (mirroring the one `W-EP-01` already carries) is a v0.6 candidate.
:::
...
| HPT blade weakener reports | `packs/nasa-7009b/examples/aerospace/uofa-aero-cou{1,2}-nasa7009b.jsonld` | **Stored `WeakenerAnnotation` records, not UofA packages** — they carry no `UnitOfAssurance`, so the rule engine cannot assess them. See the withdrawn step above |
