...
Profile dispatch (Phase 2.5 v0.5.15):
...
- If `conformsToProfile` is `https://uofa.net/vocab#ProfileComplete`: in
  addition to the minimal fields, also emit:
    `hasSensitivityAnalysis` (inline SensitivityAnalysis object),
    `hasContextOfUse.hasApplicabilityConstraint` (inline object),
    `hasContextOfUse.hasOperatingEnvelope` (inline object).
